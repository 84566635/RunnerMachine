 ...
 * aa 55：帧头
 * 66：发送卡信息的命令
 * card type：4byte + 5byte
 * card UID：6byte + 7byte + 8byte + 9byte
 * ff：为帧尾
 */
extern uchar xdata Card_Info[10] = {0xaa, 0x55, 0x66, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff};
 ...
 */
uchar xdata GymInfoBuff[GymInfoSize] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void UART4_Isr() interrupt 18
{
    ...
    if (S4CON & 0x01)
    {
        ...
        TimeGymInfo = 0;
        if(GetMileage(GymInfoBuff, MileageInfo, Buffer))//100字节接收完毕之后，跟上卡号
		{
			for(i = 0;i < 10;i ++)
				S2BUF = Card_Info[i];
